...
function IssueInvoice(props) {
    ...
    useEffect(() => {
        axios.get(' http://localhost:8080/api/auth/invoices/' + props.id).then((res) => {
            const data = res.data.items;
            seteditIssueInvoice(res.data);
            axios.post(' http://localhost:8080/api/auth/productCart', data).then((res) => {
    ...
    return (
        <div className={styles1.popup_form}>
            <button className={styles1.button_add} onClick={openForm}>
                Issue Invoice
            </button>
            {isOpen && (
                                ...
                                <View style={styles.section}>
                                    ...
                                    <View style={styles.table}>
                                        ...
                                        <View style={styles.tableRow}>
                                            ...
                                            <Text style={styles.tableCell}>
                                                <Text style={styles.totalAmount}>{IssueInvoice.totalAmount}</Text>
                                            </Text>
                                        </View>
                                    </View>
                                </View>
