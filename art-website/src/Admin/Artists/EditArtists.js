...
function EditArtists(props) {
    ...
    useEffect(() => {
        axios.get(' http://localhost:8080/api/auth/artistDetails/' + props.id).then((res) => {
            seteditArtists({
                name: res.data.name,
    ...
    const handleSubmit = (e) => {
        ...
        if (!flag) {
            ...
            axios
                .put(' http://localhost:8080/api/auth/Admin/UpdateArtists/' + props.id, data)
                .then((res) => {
                ...
                <div className={styles.overlay}>
                    <div className={styles.popup}>
                        <form className={styles.form_container} onSubmit={handleSubmit}>
                            ...
                            <input type="text" name="name" value={editArtists.name} onChange={handleChange} required />
                            <label htmlFor="biography">
                                <b>Biography</b>
                            </label>
                            <input type="text" name="biography" value={editArtists.biography} onChange={handleChange} required />
                            <label htmlFor="email">
                                <b>Email</b>
                            </label>
                            <input type="text" name="email" value={editArtists.email} onChange={handleChange} required />
                            <label htmlFor="phone">
                                <b>Phone</b>
                            </label>
                            <input type="text" name="phone" value={editArtists.phone} onChange={handleChange} required />
