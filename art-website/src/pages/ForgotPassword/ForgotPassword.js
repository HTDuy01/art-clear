...
function ForgotPassword() {
    ...
    const handleSubmit = (e) => {
        ...
        if (validateEmail(email)) {
            ...
            axios
                .post('http://localhost:8080/api/auth/forgotpassword', encodedData)
                .then((res) => {
        ...
        <>
            <form className={cx('container_form')} onSubmit={handleSubmit}>
                <fieldset className={cx('account_info')}>
                    ...
                    <label>
                        Email
                        <input name="email" type="email" placeholder="Nhập email" value={email} onChange={handleEmailChange} className={!isValid ? 'invalid' : ''} />
                    ...
                    <div className={cx('popup')}>
                        <div className={cx('form_container')}>
                            ...
                            <div className={cx('form_button')}>
                                <button type="button" onClick={closeForm} className={cx('btn')}>
                                    No
                                </button>
                                <button type="button" className={cx('btn')}>
                                    <Link to={config.routes.login}>Next</Link>
                                </button>
                            </div>
                        </div>
                    </div>
